...
export function extractTailwindBgColor(cls: string): string | null {
  const tokens = cls.split(/\s+/);
  for (const token of tokens) {
    const bare = token.replace(/^[a-z0-9-]+:/, '');
    if (bare === 'bg-white') return TAILWIND_COLORS['white'];
    if (bare === 'bg-black') return TAILWIND_COLORS['black'];
    const m = bare.match(/^bg-([a-z]+-\d{2,3})$/);
    if (m && TAILWIND_COLORS[m[1]]) return TAILWIND_COLORS[m[1]];
  }
  return null;
}

export function extractTailwindColors(html: string): RawColor[] {
  const results: RawColor[] = [];
  const attrRegex = /\bclass\s*=\s*["']([^"']+)["']/gi;
  let attrMatch: RegExpExecArray | null;

  while ((attrMatch = attrRegex.exec(html)) !== null) {
    for (const token of attrMatch[1].split(/\s+/)) {
      const bare = token.replace(/^[a-z0-9-]+:/, '');

      if (bare === 'bg-white') { results.push({ property: 'background-color', value: TAILWIND_COLORS['white'] }); continue; }
      if (bare === 'bg-black') { results.push({ property: 'background-color', value: TAILWIND_COLORS['black'] }); continue; }
      const bgM = bare.match(/^bg-([a-z]+-\d{2,3})$/);
      if (bgM && TAILWIND_COLORS[bgM[1]]) { results.push({ property: 'background-color', value: TAILWIND_COLORS[bgM[1]] }); continue; }

      if (bare === 'text-white') { results.push({ property: 'color', value: TAILWIND_COLORS['white'] }); continue; }
      if (bare === 'text-black') { results.push({ property: 'color', value: TAILWIND_COLORS['black'] }); continue; }
      const txtM = bare.match(/^text-([a-z]+-\d{2,3})$/);
      if (txtM && TAILWIND_COLORS[txtM[1]]) { results.push({ property: 'color', value: TAILWIND_COLORS[txtM[1]] }); }
    }
  }
  return results;
}
